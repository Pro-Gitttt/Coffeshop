...
class AuthRepository {
  ...
  Future<Result<UserModel>> signIn(String email, String password) async {
    ...
    try {
      debugPrint('[AuthRepository] Step 1: Calling _authService.signInWithEmailAndPassword()...');
      ...
      debugPrint('[AuthRepository] ⚠️ User is null, checking Firebase Auth session...');
      ...
      if (current != null) {
        ...
        try {
          ...
          debugPrint('[AuthRepository] ⚠️ ensureCurrentUserDocument failed: $e');
        ...
        final fallbackModel = UserModel(
          ...
          role: isAdminEmail(current.email) ? 'admin' : 'customer',
  ...
  Future<Result<UserModel>> signUp(String email, String password, {String? displayName}) async {
    try {
      debugPrint('SignUp called with email: $email, displayName: $displayName');
      final user =
          await _authService.signUpWithEmailAndPassword(email, password, displayName: displayName);
      if (user != null) {
        debugPrint('SignUp successful: ${user.uid}, email: ${user.email}, displayName: ${user.displayName}');
        return Result.ok(user);
      }
      debugPrint('SignUp failed: user is null');
      return Result.fail(code: 'unknown', message: 'Sign up failed: user not created');
    } on AppException catch (e) {
      ...
      if (currentUser != null) {
        debugPrint('User exists in Firebase Auth despite AppException, returning success');
        final role = isAdminEmail(currentUser.email) ? 'admin' : 'customer';
      ...
      if (currentUser != null) {
        ...
        debugPrint('User exists in Firebase Auth, returning success despite error');
        final role = isAdminEmail(currentUser.email) ? 'admin' : 'customer';
        return Result.ok(UserModel(
          uid: currentUser.uid,
          email: currentUser.email ?? email,
